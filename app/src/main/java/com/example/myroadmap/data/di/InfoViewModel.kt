package com.example.myroadmap.data.di
...
class InfoViewModel (private val locationRepository: InfoRepository) : ViewModel() {
    ...
    suspend fun fetchRoutes(authKey: String, origin: String, destination: String): List<RouteResponse> {
    ...
    suspend fun fetchDistanceTime(authKey: String, origin: String, destination: String): DistanceTimeResponse {
        return locationRepository.fetchDistanceTime(authKey, origin, destination).also { (dist, time) ->
